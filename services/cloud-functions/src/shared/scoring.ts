import type { MatchCandidate, NeedDocument, VolunteerDocument } from "./models.js";

export function computePriorityScore(need: NeedDocument, unmetHours: number) {
  ...
  return roundScore(
    urgency * 0.45 + unmet * 0.25 + peopleAffected * 0.2 - reviewPenalty + 0.1
  );
}

export function rankVolunteerMatches(
  need: NeedDocument,
  volunteers: Array<VolunteerDocument & { id: string }>
): MatchCandidate[] {
  return volunteers
    .filter((volunteer) => volunteer.availability_status !== "offline")
    .map((volunteer) => {
      const skillFit = volunteer.skills.some(
        (skill) => skill.toLowerCase() === need.need_type.toLowerCase()
      )
        ? 1
        : 0.35;
      ...
      const distanceKm = haversineKm(
        need.location.lat,
        need.location.lng,
        volunteer.home_location.lat,
        volunteer.home_location.lng
      );
